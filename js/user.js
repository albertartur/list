...
$('#list').click(function() {
		...
		$('.top_products_list').children('.name_list').addClass('col-md-7');
		$('.ord_products_list').children('.img_list').addClass('col-md-3');
		$('.ord_products_list').children('.name_list').addClass('col-md-7');
	...
	$('#grid').click(function() {
		...
		$('.top_products_grid').children('.name_list').removeClass('col-md-7');
		$('.ord_products_grid').children('.img_list').removeClass('col-md-3');
		$('.ord_products_grid').children('.name_list').removeClass('col-md-7');
	});
...
////////stugi vor klory mug ani
	$('#add_preview').click(function(){
			...
			let messel='';
			$('input').each(function(){
		    	if($(this).attr('id')!='search' &&  $(this).attr('id')!='client_id' && $(this).attr('id')!='id_photos'){
		    		if($(this).val().length==0){
		    			al=true;
		    			mes+=$(this).attr('id').slice(3)+', ';
		    		}
		    	}
		    })
		    $('select').each(function(){
		    	if($(this).attr('id')!='id_currency'){
		    		if($(this).val()==0){
		    	   			al=true;
		    	    		messel+=$(this).attr('id').slice(3)+',';
		    	   	}
				}
			})
			if($('#id_description').val()==0){
		    	   	al=true;
		    		messel+=$(this).attr('id').slice(3)+', ';
		    }
				if(mes!=0)
		    		mes='Please fill '+mes+' inputs';
		    	if(messel!=0)
		    		messel='Please fill '+messel+' inputs';
		   	if(al){
		   		let warnin = '<span style="color:red;">  '+mes+messel+'</span>'
		   		$.alert({
...
/////////////////////////////////////////////////// 20/07
	$('#post_ads').click(function(){
		let fileInput = document.getElementById('id_photos')
		let assoc= {};//////Alisayi mot ashxatel a assoc={} depqum
		...
		formData.append('arr',arr_to_str)
	   $.ajax({
	     ...
	     success:function(elem){
	     	$.alert({
				title: 'Info',
				content: 'wait please we must check your statement..',
			});
